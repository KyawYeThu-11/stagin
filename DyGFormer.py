...
from util.utils import Data, NeighborSampler, TimeEncoder, tensor2triplet, get_neighbor_sampler
from util.utils import NeighborSampler
from util.utils import TimeEncoder

class DyGFormer(nn.Module):        
    def __init__(self, time_feat_dim: int, channel_embedding_dim: int, patch_size: int = 1, num_layers: int = 2, num_heads: int = 2,
                 dropout: float = 0.1, max_input_sequence_length: int = 512, device: str = 'cpu', sample_neighbor_strategy: str = 'uniform',
                 time_scaling_factor: int = 1):
        """
        DyGFormer model.
    ...
    def forward(self, a, sampling_endpoints):
        ...
        for i, a_subject in enumerate(tqdm(a, desc=f'for subject {i}')):
            timepoints, src_nodes, dst_nodes = tensor2triplet(a_subject, sampling_endpoints, 1 - 0.3)
